...
public class SubjectRegisterChangedDomainEventHandler(IRegisterRepository<StudentRegister> registerRepository)
    : INotificationHandler<SubjectRegisterChangedDomainEvent>
{
    public async Task Handle(SubjectRegisterChangedDomainEvent notification, CancellationToken cancellationToken)
    {
        ...
        await registerRepository.SaveAsync(key, () => Task.FromResult(new StudentRegister()
        {
