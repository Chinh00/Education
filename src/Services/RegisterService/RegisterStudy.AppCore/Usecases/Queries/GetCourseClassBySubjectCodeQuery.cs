...
public record GetCourseClassBySubjectCodeQuery(string SubjectCode) : IQuery<ListResultModel<CourseClass>>
{
        ...
        : IRequestHandler<GetCourseClassBySubjectCodeQuery, ResultModel<ListResultModel<CourseClass>>>
    {
        ...
        {
            var register = await registerRepository.GetAsync(nameof(RegisterCourseClass)) ?? null;
            if (register != null)
            {
            ...
            return ResultModel<ListResultModel<CourseClass>>.Create(ListResultModel<CourseClass>.Create([], 0, 1, 10));
        }
    }
