    ...
    : INotificationHandler<StudentPullStartedDomainEvent>
{
    ...
    public async Task Handle(StudentPullStartedDomainEvent notification, CancellationToken cancellationToken)
    {
        ...
        var semesterUrl =
            $"https://api5.tlu.edu.vn/api/Student/Semester?Filters[0].field=StudentCode&Filters[0].operator===&Includes=SubjectResults&Includes=CourseSubjects&Filters[0].value={notification?.StudentCode}";
        ...
        student.CreateStudent(result?.Data?.PersonalInformation, result?.Data?.InformationBySchool, result?.Data?.EducationPrograms);
