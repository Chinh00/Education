...
const Form_create_course_class_section_config_stage1 = ({
    ...
    // Tự điều chỉnh tổng số tín chỉ dựa vào hai trường input
    useEffect(() => {
    ...
    // Theo dõi số tín chỉ lý thuyết và thực hành để cập nhật số tiết tương ứng
    useEffect(() => {
        form.setFieldsValue({
            model: {
                ...form.getFieldValue("model"),
    ...
    useEffect(() => {
        form.setFieldsValue({
            model: {
                ...form.getFieldValue("model"),
                theorySessions: [Math.ceil(avgTheoryPeriodsPerWeek / 2), Math.ceil(avgTheoryPeriodsPerWeek / 2)]
            }
        });
    }, [avgTheoryPeriodsPerWeek]);

    useEffect(() => {
        form.setFieldsValue({
            model: {
                ...form.getFieldValue("model"),
                practiceSessions: [Math.ceil(avgPracticePeriodsPerWeek)]
            }
        });
